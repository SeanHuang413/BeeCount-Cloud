...
- 后端应用装配：`src/main.py`
- 默认 API 前缀：`/api/v1`，配置来源为 `src/config.py`；不要在业务代码中重复硬编码。
- 前端入口：`frontend/apps/web/src/main.tsx`
- Web 路由与登录态：`frontend/apps/web/src/App.tsx`
- 前端路由名称与 URL 映射：`frontend/apps/web/src/state/router.ts`；新增页面时必须同时核对这里和 `App.tsx`。
...
- Web 查询：页面或 feature → `frontend/packages/api-client` → `/api/v1/read/*` → projection 表。
- Web 写入：页面或 feature → `frontend/packages/api-client` → `/api/v1/write/*` → 同步事件 + projection → commit 后 WebSocket 通知。
- 移动端写入：`/api/v1/sync/push` → `sync_applier.py` 合并/LWW → projection；移动端读取通过 `/sync/pull` 或 full sync。
- 遇到“写入成功但另一端不可见”时，依次检查事件是否生成、projection 是否更新、事务是否提交、通知是否发送、客户端是否重新拉取；不要先用 WebSocket 补偿数据一致性。
- 遇到 ID、权限或跨账本问题时，先确认实体是 user-global 还是 ledger-scoped，再检查 `ledger_access.py`、对应 router 查询条件与 projection 主键。
- 遇到前端 401/刷新失败时，从 `api-client/src/http.ts` 和登录态上下文定位，不要在页面内增加独立 token 刷新逻辑。

## 配置与事实来源

- 环境变量入口以 `.env.example` 和 `src/config.py` 为准；新增后端配置时两处同步更新，并避免提交真实密钥、令牌、数据库或备份产物。
- Python 依赖以 `requirements.txt` 为准；前端使用 pnpm workspace，依赖版本以各 package 的 `package.json` 与 `frontend/pnpm-lock.yaml` 为准。
- 前端当前使用 React 18、React Router 7、Vite 5 和 pnpm 10；不要凭旧版 React Router API 习惯修改现有声明式路由。
- 版本展示以 `src/version.py`、`APP_VERSION`、`VITE_APP_VERSION` 及构建注入链路为准；package 版本、Git tag 和运行时显示版本不能互相替代。
- 数据库 URL 默认是 `sqlite:///./beecount.db`；切换 PostgreSQL 时必须通过配置完成，不得在实现中按本地环境写死连接信息。
...
- 开发过程中先运行与改动最相关的单测和静态检查，完成后再执行上述最低检查；若环境、依赖或外部服务导致无法运行，必须明确记录未验证项和原因，不得宣称全部通过。
- 纯文档改动无需运行完整测试，但应检查 Markdown 内容、命令、路径和事实是否与仓库一致。
- 不要通过删除缓存、数据库、锁文件或用户文件来“修复”失败；`make wipe-local` 具有破坏性，只有用户明确要求并确认目标后才能运行。
...
- `sean_cashflow-calendar`：资金日历，路由 `/app/sean-cashflow-calendar`。只读使用近 12 个月交易、流动账户余额和信用卡还款日，保守预测未来 30/60 天周期收支与还款压力；历史推测和信用卡计划必须明确标示，不得自动创建交易或跨币种直接汇总。
- `sean_anomaly-detection`：异常交易检测，路由 `/app/sean-anomaly-detection`。只读检查近 12 个月疑似重复、短时重复、金额突增、周期涨价和大额信息缺失；必须展示判断依据和关联交易，不得自动删除、改金额或重分类。
- `sean_account-reconciliation`：账户对账，路由 `/app/sean-account-reconciliation`，固定作为头像菜单“自定义功能”的最后一项。只读比较账本余额与用户输入的实际余额，分析近 30/60/90 天单笔、两笔组合及疑似重复线索；实际余额和对账结果不保存，不得自动调整账户或交易。
...
- Sean 自定义功能页面统一使用最大 `1440px` 的居中内容容器，避免在超宽屏上产生过多横向留白；不得为了统一页面宽度而逐个侵入或改造官方大型页面、面板和业务组件。
- `sean_monthly-comparison` 的近 12 个月明细在小于 `md` 的屏幕必须按月份使用独立卡片，分区展示收入、支出、各自环比、净结余和笔数；不得把桌面 7 列表格压缩到手机宽度或依赖横向滚动。`md` 及以上继续保留桌面表格，手机端点击整张月份卡片仍需打开当月明细。
- `sean_asset-trends` 的“月度变化”在小于 `md` 的屏幕必须按月份使用独立卡片，净资产单独展示，资产和负债在下方分列；不得把月份、净资产、资产和负债四列强行挤在手机同一行。`md` 及以上继续保留桌面表格。
- 顶部导航、交易记录、日历及其他官方页面统一通过公共布局使用最大 `1440px` 的居中内容容器；大屏不得铺满整个视口，手机和平板仍占满可用宽度。不要在各官方页面内重复添加宽度容器，以降低合并 upstream 时的冲突概率。
...
- 三项功能均属于云端只读辅助工具，只使用现有 read API；不得为它们修改移动端同步协议、`sync_changes`、projection、账户余额算法或官方交易写入逻辑。
- 三项功能默认读取有限时间范围并在浏览器计算。当前单页最多读取 10,000 笔交易；若未来改变上限、迁移到后端聚合或出现数据截断，页面必须明确提示实际分析范围，不能静默给出“不完整但看似完整”的结果。

#### 资金日历

- 资金日历只预测未来 30/60 天。周期收支必须至少有 3 个月近期、相对连续且金额稳定的记录；当前金额波动阈值约为 25%，不得因一两笔同名交易就生成预测。
- 周期收支名称只取交易备注或云端分类名称，不得从附件、自由文本语义或交易备注之外猜测商户关系。
- “历史推测”和“信用卡计划”必须在界面明确区分；预测结果不是实际账单，不得自动创建或修改交易。
- 当前可用余额只统计现金、储蓄卡、支付宝和微信等流动账户；信用卡、贷款、投资、房产等不得计入可用余额。
- 信用卡计划只在账户设置了 `payment_due_day` 且当前确有负余额欠款时生成，金额按当前欠款计算，且只安排下一次还款日。
- 资金日历只汇总本位币。其他币种必须列出忽略提示，不得按 1:1 或缺失汇率直接相加。

#### 异常交易检测

- 异常检测只分析近 12 个月正常收支交易；转账和 `exclude_from_stats` 交易不参与金额异常与涨价判断。
- 高风险仅用于同日同账户、同类型、同名称和同金额的疑似重复，以及 15 分钟内的短时重复；“高风险”仍不代表一定错误。
- 金额突增至少需要 3 笔历史基线，当前阈值为近期典型金额的 2.5 倍，且绝对增量至少 100；周期涨价需要至少跨 3 个月，涨幅至少 20% 且绝对增量至少 5。
- 大额信息缺失只在金额至少 1,000 且分类、账户、备注中至少缺少两项时提示，避免把普通简记误判为异常。
- 每项异常必须展示可解释原因和关联交易；不得自动删除、改金额、切换账户、重分类或写入“已处理”状态。
- 重复检测应保持按交易指纹索引的线性扫描，不要退化为对全部交易进行平方级两两比较。

#### 账户对账

- 账户对账固定为头像菜单“自定义功能”的最后一项；除非用户明确要求重新规划菜单，否则后续新增自定义功能也不得把它移离末尾。
- 仅现金、银行卡、信用卡、支付宝、微信和普通可流水账户参与对账；房产、车辆、投资、保险、社保、贷款等估值账户不进入选择列表。
- 差额公式固定为：`实际余额 - 账本余额`。差额为负表示账本余额偏高，差额为正表示账本余额偏低；页面必须同时展示公式和方向说明。
- 信用卡沿用系统负债符号：欠款必须按负数输入，例如欠款 2,400 元输入 `-2400`；不得在对账页擅自改成正数欠款口径。
- 用户输入的实际余额只存在于当前页面状态，关闭或切换账户后清空；不得保存对账历史、未处理差额或“已对账”状态。
- 线索仅分析近 30/60/90 天，包含单笔金额接近、最多两笔合计接近及同日疑似重复；金额接近容差为 2% 或 0.01 中的较大值。
- 对账线索只是人工核对入口，不能声称已找到差额原因。处理动作必须进入官方交易页面查看、补记或编辑；不得通过生成“余额调整”交易来强行对平。
