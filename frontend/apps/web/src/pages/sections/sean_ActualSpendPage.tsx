...
export function SeanActualSpendPage() {
  ...
  return <div className="space-y-5"><div className="flex flex-wrap items-start justify-between gap-3"><div><h1 className="text-2xl font-bold tracking-tight">实际支出</h1><p className="mt-1 text-sm text-muted-foreground">按月查看预算支出扣除报销后的只读结果，不改变 App 或预算原始数据。</p></div><div className="flex items-center gap-2"><MonthSwitcher value={selectedMonth} max={currentMonth()} onChange={setSelectedMonth}/><Button variant="outline" disabled={loading} onClick={() => void load()}>刷新数据</Button></div></div>{error ? <Card><CardContent className="py-8 text-sm text-destructive">数据加载失败，请稍后重试。</CardContent></Card> : null}<BudgetReimbursementCard transactions={transactions} categories={categories} budgets={budgets} currency={currency} /></div>
}
